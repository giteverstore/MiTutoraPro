...
function account(timestamp) {
  return { currency: CURRENCY, pendingBalanceMinor: 0, availableBalanceMinor: 0, reservedBalanceMinor: 0, lifetimeCreditedMinor: 0, createdAt: timestamp, updatedAt: timestamp, schemaVersion: SCHEMA_VERSION };
}
function validateAccount(value) {
  const normalized = { ...value, reservedBalanceMinor: value.reservedBalanceMinor ?? 0 };
  if (normalized.currency !== CURRENCY || !['pendingBalanceMinor', 'availableBalanceMinor', 'reservedBalanceMinor', 'lifetimeCreditedMinor'].every((field) => Number.isSafeInteger(normalized[field]) && normalized[field] >= 0)) {
...
export function reconcileWalletProjection(wallet, transactions, withdrawals = []) {
  ...
  const total = credits.reduce((sum, entry) => sum + positiveMinor(entry.amountMinor), 0);
  ...
  const expectedAvailable = total - reserved + released;
  const pendingWithdrawals = withdrawals.filter((entry) => entry.status === 'PENDING').reduce((sum, entry) => sum + positiveMinor(entry.amountMinor), 0);
  const withdrawalMatch = withdrawals.length === 0 || pendingWithdrawals === expectedReserved;
  return { reconciled: expectedReserved >= 0 && expectedAvailable >= 0 && valid.availableBalanceMinor === expectedAvailable && valid.reservedBalanceMinor === expectedReserved && valid.lifetimeCreditedMinor === total && withdrawalMatch, availableLedgerMinor: expectedAvailable, reservedLedgerMinor: expectedReserved, lifetimeLedgerMinor: total, availableBalanceMinor: valid.availableBalanceMinor, reservedBalanceMinor: valid.reservedBalanceMinor, lifetimeCreditedMinor: valid.lifetimeCreditedMinor };
}
...
export class WalletService {
  ...
  async #settle(evidence) {
    ...
    return this.db.runTransaction(async (tx) => {
      ...
      const next = validateAccount({ ...current, availableBalanceMinor: current.availableBalanceMinor + amountMinor, lifetimeCreditedMinor: current.lifetimeCreditedMinor + amountMinor, updatedAt: timestamp });
      const ledger = { transactionId, type: 'REFERRAL_REWARD', amountMinor, currency: CURRENCY, balanceBucket: 'AVAILABLE', sourceType: 'REFERRAL', sourceId: evidence.referralId, idempotencyKey: idempotencyId, createdAt: timestamp, schemaVersion: SCHEMA_VERSION };
...
export const WALLET_CONSTANTS = Object.freeze({ currency: CURRENCY, schemaVersion: SCHEMA_VERSION, syntheticEvidenceType: SYNTHETIC_EVIDENCE });
