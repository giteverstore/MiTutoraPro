...
The catalog is metadata-first: Firestore returns deterministic 24-item cursor pages, cards render without Storage downloads, and only a selected question loads its full JSON. The active publication supplies filter facets and version binding. Load More preserves prior pages, de-duplicates IDs, and resets its cursor when filters change. Selected-content errors are retryable and never erase the catalog.
