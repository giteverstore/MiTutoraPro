...
export class PhpWorkerClient {
  ...
  execute({ source, stdin = '', filename = 'main.php', signal, timeoutMs = this.timeoutMs }) {
    if (signal?.aborted) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = timeoutMs > 0
        ? setTimeout(
          () => finish(reject, new Error(`PHP execution exceeded ${timeoutMs} ms.`)),
          timeoutMs,
        )
        : null;
      ...
      worker.addEventListener('error', (event) => finish(
        reject,
        new Error(event.message || 'The PHP runtime worker failed.'),
      ), { once: true });
