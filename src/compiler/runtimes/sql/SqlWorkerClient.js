...
export class SqlWorkerClient {
  constructor({
    ...
    workerFactory = () => new Worker(new URL('./sql.worker.js', import.meta.url), {
      type: 'module',
      name: 'ycoders-sql-runtime',
    }),
  } = {}) {
  ...
  execute({ source, setupSql = '', signal, timeoutMs = this.timeoutMs }) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = timeoutMs > 0 ? setTimeout(
        () => finish(reject, new Error(`SQL execution exceeded ${timeoutMs} ms.`)), timeoutMs,
      ) : null;
      worker.addEventListener('message', ({ data }) => finish(resolve, data), { once: true });
      worker.addEventListener('error', (event) => finish(reject, new Error(event.message || 'The SQL runtime worker failed.')), { once: true });
      signal?.addEventListener('abort', abort, { once: true });
      this.cancelActive = abort;
      worker.postMessage({ type: 'execute', source: String(source ?? ''), setupSql: String(setupSql ?? '') });
    });
  }

  reset() {
    if (this.cancelActive) this.cancelActive();
    else { this.activeWorker?.terminate(); this.activeWorker = null; }
  }
