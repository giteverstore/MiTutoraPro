import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { PracticeDetail } from './PracticeDetail';
import { PracticeFilters } from './PracticeFilters';
import { PracticePagination } from './PracticePagination';
import { PracticeQuestionCard } from './PracticeQuestionCard';
import { PracticeStatistics } from './PracticeStatistics';
import { initiallySolvedQuestionIds, practiceStatistics } from './practiceData';
import { practiceContentSource } from './practiceContentSource';
import { getPracticeDiagnostic, PRACTICE_DIAGNOSTIC_STAGES } from './practiceDiagnostics';

const initialFilters = { difficulty: 'all', topic: 'all', search: '' };
const PAGE_CACHE_LIMIT = 5;
const uniqueValues = (items, property) => [...new Set(items.map((item) => item[property]))].sort();

function retainNearestPages(map, activePage, limit = PAGE_CACHE_LIMIT) {
  if (map.size <= limit) return;
  const removable = [...map.keys()]
    .filter((page) => page !== activePage && page !== 1)
    .sort((left, right) => Math.abs(right - activePage) - Math.abs(left - activePage));
  while (map.size > limit && removable.length) map.delete(removable.shift());
}

function scrollCatalogIntoView() {
  const reducedMotion = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches;
  document.querySelector('.practice-question-list')?.scrollIntoView({ behavior: reducedMotion ? 'auto' : 'smooth', block: 'start' });
}

export function PracticePage({ initialQuestionId = null, onQuestionChange = () => {} }) {
  const [filters, setFilters] = useState(initialFilters);
  const [catalog, setCatalog] = useState({ items: [], currentPage: 1, reachablePageCount: 1, hasMore: false, loading: true, error: null, facets: null });
  const [openQuestionId, setOpenQuestionId] = useState(initialQuestionId);
  const [openQuestion, setOpenQuestion] = useState(null);
  const [questionState, setQuestionState] = useState({ loading: false, error: null, retry: 0 });
  const [solvedQuestionIds, setSolvedQuestionIds] = useState(() => new Set(initiallySolvedQuestionIds));
  const catalogRequest = useRef(0);
  const pageCache = useRef(new Map());
  const pageCursors = useRef(new Map([[1, null]]));
  const reachablePageCount = useRef(1);
  const filterOptions = useMemo(() => ({
    difficulties: catalog.facets?.difficulties ?? uniqueValues(catalog.items, 'difficulty'),
    topics: catalog.facets?.topics ?? uniqueValues(catalog.items, 'topic'),
  }), [catalog.facets, catalog.items]);

  const loadPage = useCallback(async (pageNumber = 1, { force = false, scroll = false } = {}) => {
    const requestId = ++catalogRequest.current;
    const cachedPage = pageCache.current.get(pageNumber);
    if (cachedPage && !force) {
      setCatalog((current) => ({ ...cachedPage, reachablePageCount: reachablePageCount.current, loading: false, error: null, facets: cachedPage.facets ?? current.facets }));
      if (scroll) scrollCatalogIntoView();
      return;
    }
    setCatalog((current) => ({ ...current, loading: true, error: null }));
    try {
      let pageCursor = pageCursors.current.get(pageNumber);
      if (pageNumber > 1 && !pageCursors.current.has(pageNumber)) {
        const nearestKnownPage = Math.max(...[...pageCursors.current.keys()].filter((knownPage) => knownPage < pageNumber));
        pageCursor = pageCursors.current.get(nearestKnownPage);
        for (let traversedPage = nearestKnownPage; traversedPage < pageNumber; traversedPage += 1) {
          const traversed = await practiceContentSource.listPage({ cursor: pageCursor, filters });
          if (requestId !== catalogRequest.current || !traversed.hasMore || !traversed.cursor) return;
          pageCursor = traversed.cursor;
          pageCursors.current.set(traversedPage + 1, pageCursor);
        }
      }
      const page = await practiceContentSource.listPage({ cursor: pageCursor, filters });
      setCatalog((current) => {
        if (requestId !== catalogRequest.current) return current;
        reachablePageCount.current = Math.max(reachablePageCount.current, pageNumber + (page.hasMore ? 1 : 0));
        const nextCatalog = {
          items: page.items,
          currentPage: pageNumber,
          reachablePageCount: reachablePageCount.current,
          hasMore: page.hasMore,
          loading: false,
          error: null,
          facets: page.facets ?? current.facets,
        };
        pageCache.current.set(pageNumber, nextCatalog);
        if (page.hasMore && page.cursor) pageCursors.current.set(pageNumber + 1, page.cursor);
        retainNearestPages(pageCache.current, pageNumber);
        retainNearestPages(pageCursors.current, pageNumber, PAGE_CACHE_LIMIT + 2);
        return nextCatalog;
      });
      if (requestId === catalogRequest.current && scroll) scrollCatalogIntoView();
    } catch (error) {
      if (requestId === catalogRequest.current) setCatalog((current) => ({ ...current, loading: false, error }));
    }
  }, [filters]);

  useEffect(() => {
    pageCache.current.clear();
    pageCursors.current = new Map([[1, null]]);
    reachablePageCount.current = 1;
    setCatalog((current) => ({ ...current, currentPage: 1, reachablePageCount: 1, hasMore: false }));
    loadPage(1);
  }, [loadPage]);

  useEffect(() => {
    if (!openQuestionId) { setOpenQuestion(null); setQuestionState({ loading: false, error: null, retry: 0 }); return undefined; }
    let active = true;
    const metadata = catalog.items.find(({ id }) => id === openQuestionId);
    setQuestionState((state) => ({ ...state, loading: true, error: null }));
    (metadata ? practiceContentSource.loadQuestion(metadata) : practiceContentSource.loadQuestionById(openQuestionId)).then(
      (question) => { if (active) { setOpenQuestion(question); setQuestionState((state) => ({ ...state, loading: false })); } },
      (error) => { if (active) setQuestionState((state) => ({ ...state, loading: false, error })); },
    );
    return () => { active = false; };
  }, [openQuestionId, questionState.retry]);

  if ((catalog.loading || catalog.error) && !catalog.items.length && !openQuestionId) {
    const diagnostic = catalog.error ? getPracticeDiagnostic(catalog.error, PRACTICE_DIAGNOSTIC_STAGES.metadataQuery) : null;
    return <div className="practice-page"><header className="practice-page-heading" data-practice-error-stage={diagnostic?.stage} data-practice-error-code={diagnostic?.code} data-practice-error-category={diagnostic?.category}><h1>Sharpen your coding skills.</h1><p>{catalog.error ? 'Practice questions could not be loaded. Check your connection and retry.' : 'Loading practice questions…'}</p>{catalog.error ? <button className="button button--secondary" type="button" onClick={() => loadPage(1, { force: true })}>Retry</button> : null}</header></div>;
  }
  if (openQuestionId && (questionState.loading || questionState.error)) {
    const diagnostic = questionState.error ? getPracticeDiagnostic(questionState.error, PRACTICE_DIAGNOSTIC_STAGES.storageDownload) : null;
    return <div className="practice-page"><header className="practice-page-heading" data-practice-error-stage={diagnostic?.stage} data-practice-error-code={diagnostic?.code} data-practice-error-category={diagnostic?.category}><h1>{questionState.error ? 'This question couldn’t be loaded.' : 'Loading question…'}</h1><p>{questionState.error ? 'The catalog remains available. Retry this question or return to Practice.' : 'Verifying and preparing the question content.'}</p>{questionState.error ? <button className="button button--secondary" type="button" onClick={() => setQuestionState((state) => ({ ...state, retry: state.retry + 1 }))}>Retry</button> : null}<button className="button button--ghost" type="button" onClick={() => setOpenQuestionId(null)}>Back to Practice</button></header></div>;
  }
  if (openQuestion) {
    return <PracticeDetail question={openQuestion} solved={solvedQuestionIds.has(openQuestion.id)} onBack={() => { setOpenQuestionId(null); onQuestionChange(null); }} onComplete={(questionId) => setSolvedQuestionIds((current) => new Set(current).add(questionId))} />;
  }

  return (
    <div className="practice-page">
      <header className="practice-page-heading"><h1>Sharpen your coding skills.</h1><p>Choose a focused problem, write real Python, and validate your output.</p></header>
      <PracticeFilters filters={filters} options={filterOptions} onChange={(key, value) => setFilters((current) => ({ ...current, [key]: value }))} />
      <div className="practice-catalog"><section className="practice-question-list" aria-labelledby="practice-question-list-title" aria-busy={catalog.loading}><header><div><span>Question Catalog</span><h2 id="practice-question-list-title">{catalog.items.length} questions</h2></div></header><div>
        {catalog.items.map((question) => <PracticeQuestionCard question={question} solved={solvedQuestionIds.has(question.id)} onSelect={(nextQuestion) => { setOpenQuestionId(nextQuestion.id); onQuestionChange(nextQuestion.id); }} key={question.id} />)}
        {!catalog.items.length ? <p className="practice-no-results">No questions match your filters.</p> : null}
        {catalog.error ? <p className="practice-no-results">This page couldn’t be loaded. <button type="button" onClick={() => loadPage(catalog.currentPage, { force: true })}>Retry</button></p> : null}
      </div></section></div>
      {catalog.items.length ? <PracticePagination currentPage={catalog.currentPage} reachablePageCount={catalog.reachablePageCount} hasNextPage={catalog.hasMore} onPageChange={(pageNumber) => loadPage(pageNumber, { scroll: true })} /> : null}
      <PracticeStatistics statistics={practiceStatistics} language={catalog.items[0]?.language ?? 'Python'} />
    </div>
  );
}
