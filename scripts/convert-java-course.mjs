...
const course = { $schema: '../../schemas/learning-course.schema.json', schemaVersion: '1.0.0', id: 'java', slug: 'java', title: 'Java Basics', description: 'Build a practical Java foundation through structured explanations, examples, quizzes, and programming problems.', locale: 'en-US', status: 'published', metadata: { version: '1.0.0', authors: [{ name: 'ycoders' }], level: 'beginner', estimatedMinutes, tags: ['java', 'programming', 'fundamentals'], updatedAt: '2026-08-21T00:00:00.000Z' }, navigation: { defaultLessonId: lessons[0].id, sequence: 'module-order', skipLockedLessons: true, labels: { previous: 'Previous lesson', next: 'Next lesson', progress: 'Course progress', currentLesson: 'Current lesson' } }, modules };
...
const local = JSON.parse(await readFile(LOCAL_METADATA, 'utf8')); local.courses = [...local.courses.filter(({ id }) => id !== 'java'), { id: 'java', title: course.title, description: course.description, version: '1.0.0', publishedVersion: 'v1', source: '/courses/java-course.json' }]; await writeFile(LOCAL_METADATA, `${JSON.stringify(local, null, 2)}\n`);
